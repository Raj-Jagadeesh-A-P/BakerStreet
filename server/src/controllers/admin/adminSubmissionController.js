...
import { events, subs, teams, cases } from '../../db/repo.js';
...
export const listSubmissions = asyncHandler(async (req, res) => {
  ...
  const { caseId, correct, teamId } = req.query;
  ...
  for (const r of slice) {
    const [team, caseRow] = await Promise.all([teams.get(r.teamId), cases.get(r.caseId)]);
    out.push({
      id: r.id,
      answer: r.answer,
      correct: r.correct,
      ...
      case: { id: r.caseId, title: caseRow?.title ?? '?', order: caseRow?.order ?? 0, finalCase: caseRow?.finalCase ?? false },
    });
  }
